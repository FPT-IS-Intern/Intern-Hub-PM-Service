...
public class TeamMemberServiceImpl implements TeamMemberService {
        ...
        public List<TeamMemberResponse> addMembers(Long teamId, List<TeamMemberCreateRequest> requests) {
                ...
                Set<Long> existingMemberIds = teamMemberRepository.findAllByTeamIdAndStatus(teamId, Status.ACTIVE)
                                .stream()
                ...
                List<TeamMember> members = requests.stream().map(request -> {
                        ...
                        if (!projectMemberIds.contains(userId)) {
                                throw new ConflictDataException(
                                                "User ID " + userId + " không phải là thành viên của dự án này");
                        }
                ...
                return savedMembers.stream()
                                .map(member -> toResponse(member, userDetailMap.get(member.getUserId()),
                                                taskCountMap.getOrDefault(member.getUserId(), 0L)))
                                .toList();
        }
        ...
        public PaginatedData<TeamMemberResponse> getMembers(Long teamId, String keyword, int page, int size) {
                ...
                if (kw.isEmpty()) {
                        ...
                        Page<TeamMember> memberPage = teamMemberRepository.findAllByTeamIdAndStatus(teamId,
                ...
                List<TeamMember> allMembers = teamMemberRepository.findAllByTeamIdAndStatus(teamId, Status.ACTIVE);
                ...
                if (member.getTeam().getStatus() != StatusWork.CANCELED) {
                        ...
                        if (taskCount > 0) {
                                throw new BadRequestException(
                                                "Không thể xóa thành viên đã được phân công công việc trong team");
                        }
                }
        ...
        private TeamMemberResponse toResponse(TeamMember member, HrmUserClientModel userDetail, Long taskCount) {
                ...
                return TeamMemberResponse.builder()
                                ...
                                .fullName(userDetail != null ? userDetail.fullName()
        ...
        private Map<Long, Long> getTaskCountMap(Long teamId, List<Long> userIds) {
                ...
                List<Object[]> counts = taskRepository.countTasksByTeamIdAndAssigneeIds(teamId, userIds,
                                StatusWork.CANCELED);
